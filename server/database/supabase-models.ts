...
export class SupabaseDatabase {
  ...
  async getCustomerCurrentMonthBill(customerId: number): Promise<{
    ...
    console.log(
      `ROBUST Billing for Customer ${customerId} (${customer.name}):`,
      {
        deliveredDays,
        deliveredAmount,
        remainingDays,
        ...
        totalDeliveriesCount: deliveries?.length || 0,
        invalidDeliveriesCount:
          (deliveries?.length || 0) - validDeliveries.length,
      },
    );
